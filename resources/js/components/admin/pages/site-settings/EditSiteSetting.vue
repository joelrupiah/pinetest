                    ...
                    <div class="col-md-12">
                      <div class="form-group mb-3">
                        <el-button type="success" :loading="loading" size="mini" 
                            @click.prevent="updateSiteSetting()">{{ loading ? 'Updating settings.....' : 'Update Site Settings' }}
                        </el-button>
...
export default {
    ...
    methods: {
        getSiteSetting: async function(){
            Api().get('/admin/get-site-setting/'+this.$route.params.id)
                .then((response) => {
                    this.form.id = response.data.sitesetting.id,
                    this.form.site_email = response.data.sitesetting.email,
                    this.form.site_phone = response.data.sitesetting.phone,
                    this.form.site_town = response.data.sitesetting.town,
                    this.form.site_address = response.data.sitesetting.address,
                    this.form.opening_time = response.data.sitesetting.opening_time,
                    this.form.closing_time = response.data.sitesetting.closing_time
                })
        },
        updateSiteSetting: async function(){
            this.loading = true
            Api().post('/admin/update-site-settings/'+this.$route.params.id, this.form)
                .then(() => {
                    this.loading = false
                    this.errors = {}
                    this.$notify({
