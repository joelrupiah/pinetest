                    ...
                    <div class="col-md-12">
                      <div class="form-group mb-3">
                        <el-button type="warning" :loading="loading" size="mini" 
                            @click.prevent="updateChoose()">{{ loading ? 'Updating choose data.....' : 'Update Choose' }}
                        </el-button>
...
export default {
    ...
    methods: {
        ...
        getChoose: async function(){
            Api().get('/admin/get-choose/'+this.$route.params.id)
                .then(response => {
                    const this_ = this
                    this.form.id = response.data.choose.id
                    this.form.title = response.data.choose.title
                    this.form.image = response.data.choose.image
                    this.form.description = response.data.choose.description
                })
        },
        updateChoose: async function(){
            this.loading = true
            Api().post('/admin/edit-choose/'+this.$route.params.id, this.form)
                .then(() => {
                    this.loading = false
                })
                .catch(error => {
